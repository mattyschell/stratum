-- (placeholder, I copied in from old project for possible use)
-- Motivation and De-movtivation:
-- This approximates the DDL to support http://maps.nyc.gov/taxmap/map.htm
--   (the online NYC Dept. of Finance Digital Tax Map) which is a webmap-
--   templated application reading from an Oracle database with geometries 
--   stored/edited in a maddening combo of ESRI SDE binary and Oracle 
--   sdo_geometry.
-- As of 2019 AD the  Dept. of Finance is rebuilding that application under a 
--    contract. It is not clear to me if some sort of interim data store will 
--    be necessary until that rewrite is complete.
-- If yeah, the DDL is here for reference.
-- If nah, then I think all we need is the schema-taxmap.sql in this repo to 
--    release data for public use.  That mirrors what's released via NYC Open Data.  
--
